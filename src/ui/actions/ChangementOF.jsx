...
function ChangementOF() {
  ...
  const handelSubmit = async (e) => {
    ...
    setErrorsMessage([])
    setLoading(true)
    const response = await api.post('production/change', form)
  ...
  return (
    <div>
      <CModal title="Changement d'OF" label="Changement d'OF" icon={<RefreshCcwDot/>}>
        ...
        {activeStep === 3 && (
            <div className='space-y-4'>
              <div className='p-4 border rounded-md'>
                <p className='font-medium mb-4'>Nouveau OF/OOP</p>
                <div className='flex flex-col space-y-2'>
                  <div>
                    <label
                      htmlFor='code-of'
                    ...
                    <input
                      id='code-of'
                      onChange={(e) =>
                        setForm({ ...form, code_of_change: e.target.value })
                      }
                      className='w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500'
                      placeholder='OF/OOP'
                    />
